...
function probabilities(sport, home, away) {
  ...
  if (!sport.supports_draw) {
    return { home: homeP, draw: 0, away: 1 - homeP };
  }
...
function profile(sport, participant) {
  ...
  if (sport.key === 'tennis') {
    return {
      ...
      matchesPlayed,
      form,
      serve: Math.max(50, Math.min(96, Math.round(65 + (rating - 1500) / 8))),
      return: Math.max(50, Math.min(96, Math.round(63 + (rating - 1500) / 9))),
      winRate: Math.max(35, Math.min(82, Math.round(50 + (rating - 1500) / 12))),
      surface: Math.max(50, Math.min(96, Math.round(60 + (rating - 1500) / 9)))
    };
  }
  ...
  if (sport.key === 'nfl') {
    return {
      ...
      matchesPlayed,
      form,
      offense: Math.max(40, Math.min(95, Math.round(68 + (rating - 1500) / 7))),
      defense: Math.max(40, Math.min(95, Math.round(66 + (rating - 1500) / 8))),
      efficiency: Math.max(40, Math.min(95, Math.round(64 + (rating - 1500) / 8)))
    };
  }
  ...
  if (sport.key === 'nhl') {
    return {
      ...
      matchesPlayed,
      form,
      attack: Math.max(40, Math.min(95, Math.round(68 + (rating - 1500) / 8))),
      defense: Math.max(40, Math.min(95, Math.round(66 + (rating - 1500) / 8))),
      goaltending: Math.max(40, Math.min(95, Math.round(70 + (rating - 1500) / 9)))
    };
  }
  ...
  if (sport.key === 'mlb') {
    return {
      ...
      matchesPlayed,
      form,
      offense: Math.max(40, Math.min(95, Math.round(69 + (rating - 1500) / 8))),
      pitching: Math.max(40, Math.min(95, Math.round(68 + (rating - 1500) / 8))),
      depth: Math.max(40, Math.min(95, Math.round(64 + (rating - 1500) / 9)))
    };
  }
  ...
  if (sport.key === 'nba') {
    return {
      ...
      matchesPlayed,
      form,
      offense: Math.max(40, Math.min(96, Math.round(70 + (rating - 1500) / 7))),
      defense: Math.max(40, Math.min(96, Math.round(67 + (rating - 1500) / 8))),
      tempo: Math.max(40, Math.min(96, Math.round(65 + (rating - 1500) / 9)))
    };
  }
  ...
  return {
    ...
    matchesPlayed,
    form,
    attack: Math.max(40, Math.min(95, Math.round(70 + (rating - 1500) / 8))),
    defense: Math.max(40, Math.min(95, Math.round(68 + (rating - 1500) / 8))),
    control: Math.max(40, Math.min(95, Math.round(65 + (rating - 1500) / 9)))
  };
}
...
app.post('/api/participants', (req, res) => {
  ...
  const person = getOrCreateParticipant(sport, req.body?.name);
  if (!person) return res.status(400).json({ error: 'Enter a valid name up to 80 characters.' });

  res.status(201).json({ ...person, stats: profile(sport, person) });
});
...
app.post('/api/predict', (req, res) => {
  ...
  const home = getOrCreateParticipant(sport, req.body?.home);
  const away = getOrCreateParticipant(sport, req.body?.away);
  ...
  const saved = db.prepare('INSERT INTO predictions(match_id,sport_id,home_probability,draw_probability,away_probability,model_version) VALUES (?,?,?,?,?,?)')
    .run(match.lastInsertRowid, sport.id, p.home, p.draw, p.away, 'elo-v2');

  const margin = Math.abs(p.home - p.away);
  const confidence = Math.round(Math.min(96, 52 + margin * 90));

  res.json({
    id: saved.lastInsertRowid,
    matchId: match.lastInsertRowid,
    sport: sport.key,
    home: { name: home.name, stats: profile(sport, home) },
    away: { name: away.name, stats: profile(sport, away) },
    probabilities: p,
    odds: {
      home: odds(p.home),
      draw: odds(p.draw),
    ...
    confidence,
    homeAdvantage: sport.key === 'tennis' ? 0 : 35,
    model: 'Elo v2'
  });
});
...
app.post('/api/matches/:id/result', (req, res) => {
  const match = db.prepare(`
    SELECT m.*, s.key, s.k_factor, s.supports_draw
    FROM matches m
  ...
  if (!['home', 'away', 'draw'].includes(winner) || (winner === 'draw' && !match.supports_draw)) {
    return res.status(400).json({ error: 'Invalid result for this sport.' });
  }

  const home = participantById.get(match.home_participant_id);
  const away = participantById.get(match.away_participant_id);
  const p = probabilities({ key: match.key, supports_draw: !!match.supports_draw }, home, away);
  const actual = winner === 'home' ? [1, 0] : winner === 'away' ? [0, 1] : [0.5, 0.5];
  ...
  db.transaction(() => {
    update.run(home.rating + match.k_factor * (actual[0] - p.home), home.id);
    update.run(away.rating + match.k_factor * (actual[1] - p.away), away.id);
    db.prepare("UPDATE matches SET status='final',home_score=?,away_score=? WHERE id=?").run(homeScore ?? null, awayScore ?? null, match.id);
  })();
...
app.listen(port, () => {
  console.log(`Multi-sport Elo app listening on http://localhost:${port}`);
});
